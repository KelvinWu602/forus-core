...
// go test -timeout 200s -run ^TestBlacklistPathNextHopIsNotProxyConnectFail$ github.com/KelvinWu602/forus-core/p2p -v -count=1
func TestBlacklistPathNextHopIsNotProxyConnectFail(t *testing.T) {
	// Defined in test/config-TestBlacklistPath-NextHopIsNotProxy-ConnectFail-N1.yaml
	// Defined in test/config-TestBlacklistPath-NextHopIsNotProxy-ConnectFail-N2.yaml
	// Defined in test/config-TestBlacklistPath-NextHopIsNotProxy-ConnectFail-N3.yaml
	// N1 127.0.0.1
	// N2 127.0.0.2
	// N3 127.0.0.3
	...
	go func() {
		N1 = StartNodeInternal("../unit_test_configs/config-TestBlacklistPath-NextHopIsNotProxy-ConnectFail-N1.yaml")
		setup.Done()
	}()

	go func() {
		N2 = StartNodeInternal("../unit_test_configs/config-TestBlacklistPath-NextHopIsNotProxy-ConnectFail-N2.yaml")
		setup.Done()
	}()

	go func() {
		N3 = StartNodeInternal("../unit_test_configs/config-TestBlacklistPath-NextHopIsNotProxy-ConnectFail-N3.yaml")
		setup.Done()
	}()
	setup.Wait()
...
// go test -timeout 200s -run ^TestBlacklistPathNextHopIsNotProxySuccess$ github.com/KelvinWu602/forus-core/p2p -v -count=1
func TestBlacklistPathNextHopIsNotProxySuccess(t *testing.T) {
	// Defined in test/config-TestBlacklistPath-NextHopIsNotProxy-Success-N1.yaml
	// Defined in test/config-TestBlacklistPath-NextHopIsNotProxy-Success-N2.yaml
	// Defined in test/config-TestBlacklistPath-NextHopIsNotProxy-Success-N3.yaml
	// N1 127.0.0.1
	// N2 127.0.0.2
	// N3 127.0.0.3
	...
	go func() {
		N1 = StartNodeInternal("../unit_test_configs/config-TestBlacklistPath-NextHopIsNotProxy-Success-N1.yaml")
		setup.Done()
	}()

	go func() {
		N2 = StartNodeInternal("../unit_test_configs/config-TestBlacklistPath-NextHopIsNotProxy-Success-N2.yaml")
		setup.Done()
	}()

	go func() {
		N3 = StartNodeInternal("../unit_test_configs/config-TestBlacklistPath-NextHopIsNotProxy-Success-N3.yaml")
		setup.Done()
	}()
	setup.Wait()
	...
	// N3.paths.iterate(func(pathID uuid.UUID, pathProfile PathProfile) {
	// 	assert.Equal(newPathID, pathID, "N3 should be using the old path")
	// 	assert.Equal("ImmutableStorage", pathProfile.next2, "N3's next next hop should be ImmutableStorage")
	// 	assert.Equal("127.0.0.1", pathProfile.next, "N3's next hop should be N1")
	// }, true)

	t.Log(N3.paths.data)

	// Final checking, current topology should be
	// N1 <- N3
	// N1 <- N2
	// Both paths having the same uuid
	pathOnN1, _ := N1.paths.getValue(newPathID)
	pathOnN2, _ = N2.paths.getValue(newPathID)
	pathOnN3, _ = N3.paths.getValue(newPathID)
	assert.Equal(pathOnN1.uuid, pathOnN2.uuid, "path on N1 and N2 should have the same uuid")
	assert.Equal(pathOnN2.uuid, pathOnN3.uuid, "path on N2 and N3 should have the same uuid")

	// validate path on N1
	assert.Equal("ImmutableStorage", pathOnN1.next, "N1 next hop should be ImmutableStorage")
	assert.Equal("", pathOnN1.next2, "N1 next next hop should be nothing")

	// validate path on N2
	assert.Equal("127.0.0.1", pathOnN2.next, "N2 next hop should be N1")
	assert.Equal("ImmutableStorage", pathOnN2.next2, "N2 next next hop should be ImmutableStorage")

	// validate path on N3
	assert.Equal("127.0.0.1", pathOnN3.next, "N3 next hop should be N1")
	assert.Equal("ImmutableStorage", pathOnN3.next2, "N3 next next hop should be ImmutableStorage")

	// validate covers on N1
	coverN2onN1, foundN2 := N1.covers.getValue("127.0.0.2")
	coverN3onN1, foundN3 := N1.covers.getValue("127.0.0.3")
	assert.Equal(2, N1.covers.getSize(), "N1 should have 2 covers")
	assert.Equal(true, foundN2, "N1 should have cover N2")
	assert.Equal(true, foundN3, "N1 should have cover N3")

	assert.Equal(newPathID, coverN2onN1.treeUUID, "N2 should connect to N1 proxy path")
	assert.Equal(newPathID, coverN3onN1.treeUUID, "N3 should connect to N1 proxy path")

	// N2 and N3 should have no covers
	assert.Equal(0, N2.covers.getSize(), "N2 should have 0 covers")
	assert.Equal(0, N3.covers.getSize(), "N3 should have 0 covers")
